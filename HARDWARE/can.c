...
{	   
	*ide=CAN1->sFIFOMailBox[fifox].RIR&0x04; //Gets the value of the identifier selection bit  
	...
	*rtr=CAN1->sFIFOMailBox[fifox].RIR&0x02;	//Gets the remote send request value 
	*len=CAN1->sFIFOMailBox[fifox].RDTR&0x0F; //Get the DLC 
 	//*fmi=(CAN1->sFIFOMailBox[FIFONumber].RDTR>>8)&0xFF; //Get the FMI //
	//Receive data //接收数据
  ...
  if(fifox==0)CAN1->RF0R|=0X20;      //Free the FIFO0 mailbox 
	else if(fifox==1)CAN1->RF1R|=0X20; //Free the FIFO1 mailbox  
}
/**************************************************************************
